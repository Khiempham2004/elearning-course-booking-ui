...
//                 // Tính toán thống kê
//                 const totalStudents = usersRes?.data?.filter(u => u.role === 'user')?.length || 0;
//                 const totalTeachers = usersRes?.data?.filter(u => u.role === 'teacher')?.length || 0;
//                 const totalReports = enrollmentsRes?.data?.length || 0;

//                 // Mock data cho trạng thái (nếu API không cung cấp)
...
//                 // Đặt dữ liệu mẫu cho danh sách
//                 setPendingStudents(enrollmentsRes?.data?.slice(0, 3)?.map((e, idx) => ({
//                     key: idx + 1,
//                     studentId: `SV${String(idx + 1).padStart(3, '0')}`,
//                     name: `Sinh viên ${idx + 1}`,
//                     title: e.courseId?.name || 'Chưa cập nhật',
//                     teacher: 'GV',
//                     submittedDate: new Date().toLocaleDateString('vi-VN'),
//                     defenseDate: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toLocaleDateString('vi-VN'),
//                     status: ['Pending', 'Scheduled', 'Graded'][Math.floor(Math.random() * 3)],
//                 })) || []);
